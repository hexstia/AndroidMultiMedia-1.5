...
extern "C"
JNIEXPORT jint JNICALL
Java_com_wangheart_rtmpfile_ffmpeg_FFmpegHandle_initVideo2(JNIEnv *env, jobject instance,
                                                           jstring url_) {
    const char *out_path = env->GetStringUTFChars(url_, 0);
    logd(out_path);
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_wangheart_rtmpfile_ffmpeg_FFmpegHandle_sendH264(JNIEnv *env, jobject instance,
                                                         jbyteArray buffer_, jint len) {
    int ret = 0;
    int dataLen = len;
    __android_log_print(ANDROID_LOG_WARN, "eric", "size %d", len);
//    startTime = av_gettime();
    jbyte *in = env->GetByteArrayElements(buffer_, NULL);
    //是否编码前的YUV数据
    av_new_packet(&enc_pkt, dataLen);
    memcpy(enc_pkt.data, in, dataLen); //Y
    enc_pkt.stream_index = video_st->index;
    AVRational time_base = ofmt_ctx->streams[0]->time_base;//{ 1, 1000 };
    enc_pkt.pts = count1 * (video_st->time_base.den) / ((video_st->time_base.num) * fps);
    enc_pkt.dts = enc_pkt.pts;
