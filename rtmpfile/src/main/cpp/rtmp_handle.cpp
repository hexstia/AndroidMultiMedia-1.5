...
//    if (!RTMP_SetupURL(rtmp, "rtmp://192.168.31.127/live")) {
    ...
    packet = (RTMPPacket *) malloc(sizeof(RTMPPacket));
    RTMPPacket_Alloc(packet, 344*720+256);
//    RTMPPacket_Alloc(packet, 106605);
RTMPPacket_Reset(packet);
...
//    }
    ...
    packet->m_nBodySize = datalength;
    if (!RTMP_IsConnected(rtmp)) {
        __android_log_print(ANDROID_LOG_WARN, "eric","rtmp连接失败");
        RTMP_Log(RTMP_LOGERROR, "rtmp is not connect\n");
        return -4;
    }
    if (!RTMP_SendPacket(rtmp, packet, 1)) {
        __android_log_print(ANDROID_LOG_WARN, "eric","rtmp发送失败");
        RTMP_Log(RTMP_LOGERROR, "Send Error\n");
        return -5;
    }

    env->ReleaseByteArrayElements(buf_, buf, JNI_COMMIT);
    return 0;
}
...
//Java_com_wangheart_rtmpfile_rtmp_RtmpHandle_close
extern "C"
JNIEXPORT jint JNICALL
