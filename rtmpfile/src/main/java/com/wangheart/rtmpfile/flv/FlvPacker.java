...
import static com.wangheart.rtmpfile.flv.FlvPackerHelper.AUDIO_SPECIFIC_CONFIG_SIZE;
import static com.wangheart.rtmpfile.flv.FlvPackerHelper.FLV_HEAD_SIZE;
import static com.wangheart.rtmpfile.flv.FlvPackerHelper.FLV_TAG_HEADER_SIZE;
import static com.wangheart.rtmpfile.flv.FlvPackerHelper.PRE_SIZE;
import static com.wangheart.rtmpfile.flv.FlvPackerHelper.VIDEO_HEADER_SIZE;
import static com.wangheart.rtmpfile.flv.FlvPackerHelper.VIDEO_SPECIFIC_CONFIG_EXTEND_SIZE;
...
public class FlvPacker implements Packer, AnnexbHelper.AnnexbNaluListener{
    ...
    public void onAudioData(ByteBuffer bb, MediaCodec.BufferInfo bi) {
        if(packetListener == null || !isHeaderWrite || !isKeyFrameWrite) {
            Log.i("TTTT","packetListener == null || !isHeaderWrite || !isKeyFrameWrite");

            return;
        }
//        Log.i("TTTT","comging to onAudioData() data:"+bi.size);
        bb.position(bi.offset);

        if ((bi.offset + bi.size > bb.capacity()) || (bi.offset + bi.size < 0)){
            Log.i("TTTT","comging to onAudioData() data:"+bi.size);
            Log.i("TTTT","comging to onAudioData() offset:"+bi.offset);
            Log.i("TTTT","comging to onAudioData() capacity:"+bb.capacity());
        }else{
            try {
                bb.limit(bi.offset + bi.size);
            }catch (IllegalArgumentException e){
                Log.i("TTTT","Exception  bb.limit(bi.offset + bi.size)");
                return;
            }
